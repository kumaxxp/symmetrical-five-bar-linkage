...
class InverseKinematics:
    ...
    def compute_inverse_kinematics(self):
        M1_candidates = self.calculate_M1_candidates()
        X_candidates = self.calculate_X_candidates(M1_candidates)
        M2_candidates = self.calculate_M2_candidates(X_candidates)
        ...
        for M1 in M1_candidates:
            for X in X_candidates:
                for M2 in M2_candidates:
                    if self.is_convex([self.B1, M1, X, M2, self.B2]):
                        valid_combinations.append((M1, X, M2))
    ...
    def calculate_X_candidates(self, M1_candidates):
        X_candidates = []
        for M1 in M1_candidates:
    ...
    def calculate_M2_candidates(self, X_candidates):
        M2_candidates = []
        for X in X_candidates:
            M2_candidates.extend(circle_intersections(self.B2, self.b, X, self.m))
        return M2_candidates
